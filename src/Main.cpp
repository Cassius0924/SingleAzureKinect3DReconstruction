...
//    CasWebSocket webSocket("175.178.56.40","8080","/ws-test");
//    // CasWebSocket webSocket("ws://39.108.216.190/ws");
//    //连接服务器
...
//#endif
            ...
            if (depthImage.get_width_pixels() == rgbImage.get_width_pixels() &&
                ...
                std::copy(depthImage.get_buffer(), depthImage.get_buffer() +
            ...
            k4aTransformation.depth_image_to_point_cloud(transformed_depthImage, K4A_CALIBRATION_TYPE_COLOR,
                                                         &point_cloud_image);
            ...
            pcl::io::savePLYFile("ply/" + filename_pc + ".ply", *cloud_filtered);   //将点云数据保存为ply文件
            pcl::io::savePCDFile("pcd/" + filename_pc + ".pcd", *cloud_filtered);   //将点云数据保存为pcd文件
