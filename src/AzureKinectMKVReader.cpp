...
int main(int argc, char **argv) {
    ...
    if (!utility::ProgramOptionExists(argc, argv, "--output")) {
        ...
        if (utility::filesystem::DirectoryExists(output_path)) {
            ...
            utility::filesystem::MakeDirectoryHierarchy(output_path + "/pcd");
            write_image = true;
        }
    }
    ...
    while (!mkv_reader.IsEOF() && !flag_exit) {
        if (flag_play) {
            ...
            if (write_image) {
                ...
                auto pcd_file =
                        fmt::format("{0}/pcd/{1:05d}.pcd", output_path, idx);
                utility::LogInfo("Writing to {}", pcd_file);
                auto pcd = geometry::PointCloud::CreateFromDepthImage(im_rgbd->depth_, mkv_reader.GetMetadata().intrinsics_);
