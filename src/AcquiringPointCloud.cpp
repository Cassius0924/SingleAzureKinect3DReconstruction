...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        if (device.get_capture(&capture)) {
            std::cout << i_auto << ". Capture several frames to give auto-exposure"
                      << std::endl;
    ...
    std::thread camera_thread([&]() {
        while (true) {
            ...
            image_data.rgb_image = rgb_image;
    ...
    std::thread cloud_thread([&]() {
        while (true) {
            ...
            rgb_image_item = image_data.rgb_image;
            depth_image_item = image_data.depth_image;
            ...
            for (int i = 0;
                 i < color_image_width_pixels * color_image_height_pixels; i++) {
            ...
            cloud_task_count--;
            cout << "cloud_task_count: " << cloud_task_count << endl;
