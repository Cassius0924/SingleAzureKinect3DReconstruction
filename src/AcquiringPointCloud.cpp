...
int main(int argc, char **argv) { // TODO: 可以传参，传入配置文件路径
    ...
    // 开始旋转舵机
    // int stm32_fd;
    // if ((stm32_fd = cas::bot::initMotor(STM32_SERIAL_PORT_NAME)) < 0) {
    //     return -1;
    // }

    // 判断旋转方向
    if (bot_motor.rotate(FIRST_MOTOR_ROTATION)) {
        // if (cas::bot::rotateMotor(stm32_fd, FIRST_MOTOR_ROTATION)) {
        cout << "舵机旋转成功" << endl;
    ...
    cas::bot::BotCar bot_car(BOT_CAR_SERIAL_PORT_NAME);
    ...
    thread cloud_thread([&]() {
        ...
        open3d::pipelines::registration::ICPConvergenceCriteria icp_criteria(relative_fitness, relative_rmse, max_iteration);
        ...
        while (true) {
            ...
            for (int i = 0; i < color_image_width_pixels * color_image_height_pixels; i++) {
                if (point_cloud_image_data[3 * i + 0] != 0 && point_cloud_image_data[3 * i + 1] != 0 && point_cloud_image_data[3 * i + 2] != 0) {
    ...
    thread bot_arm_thread([&]() {
        while (true) {
            ...
            // memcpy(databuff, tempbuff, recv_long);//将接收到的数据存入
            // databuff 中 write(arm_fd, databuff, recv_long);
        }
    });
    ...
    thread receive_client_thread([&]() {
        ...
        while (true) {
            ...
            // // 等待接受到客户端的数据
            recv_long = recv(client_fd, client_buffer, 1024, 0);

            // 接受到的是protobuf数据, 需要反序列化
            cas::proto::DataMessage data_message;
            if (!data_message.ParseFromArray(client_buffer, recv_long)) {
                cerr << "反序列化客户端数据失败" << endl;
            } else {
                cout << "反序列化客户端数据成功" << endl;
            }
    ...
    thread receive_stm32_thread([&]() {
        // char stm32_buffer[128];
        // while (true) {
        //     lock_guard<mutex> lock(stm32_mutex);
        //     // 等待接受到stm32的数据
        //     recv_long = recv(stm32_fd, stm32_buffer, 128, 0);
        //     // 将tempbuff 转为string
        //     string stm32_data(stm32_buffer);
        //     // "M$F DONE", "M$R DONE", "T$49.2,25.3"

        //     cout << "long: " << recv_long << endl;
        // }
    });
    ...
    while (true) {
        if (need_reconnstrcution) {
            ...
            if (abs(prev_angle.yaw) > EXIT_RADIAN) {
                ...
                camera_cv.wait(lock, [&] { return need_break; });
                ...
                if (IS_CREATE_SERVER) {
                    ...
                    cas::net::proto::send_message(client_fd, data_message);
                    mesh_message->Clear();
                    write_count++;
                    cout << "===============================" << endl;
                ...
                cas::net::proto::send_exit_mesh_message(client_fd);
